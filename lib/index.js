    ...
    input.parseOptions(optionsRaw, callbackRaw, function (options, callback) {
      ...
      file.openTemplate(templatePath, function (err, template) {
        ...
        preprocessor.execute(template, options, function (err, template) {
          ...
          // 获取图片
          let images = imageUtil.getImagesFromData(data);
          // 处理data的空格为可显示的,默认空格会被去掉,我的业务场景需要展示空格
          if (template.extension == 'docx') textUtil.spaceObject(data);
          ...
          walkFiles(template, data, options, 0, function (err, report) {
            ...
            // 执行删除元素操作
            if (template.extension == 'docx') dropUtil.dropElementToDocument(report)
              // 插入图片
            if (template.extension == 'xlsx') imageUtil.putImagesToSheets(report, images);
            if (template.extension == 'docx') imageUtil.putImagesToDocument(report, images);
            // 整体处理处理文字   字体 、加粗、大小
            if (template.extension == 'docx') textUtil.putFontStyleFullToDocument(report, optionsRaw)
            // 局部处理处理文字   字体 、加粗、大小、颜色
            if (template.extension == 'docx') textUtil.putFontStyleToDocument(report)
