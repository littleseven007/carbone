 ...
 * @param {Object} report - report data
 */
function dropElementToExcel(report) {
  const documentIndex = report.files.findIndex(
    (file) => file.name.indexOf("xl/worksheets/sheet1.xml") != -1
  );
  let documentXml = report.files[documentIndex].data;
  // 匹配是否能找到:drop(val)
  const regex = /\:drop\(.*?\)/;
  ...
  while ((item = documentXml.match(regex)) !== null) {
    ...
    let tagValue = match.value.match(/\((.*?)\)/)[1];
    const leftTag = `<${tagValue}`;
    const rightTag = `</${tagValue}>`;
    const firstTagIndex = documentXml
      .slice(0, match.index)
      .lastIndexOf(leftTag);
    const lastTagIndex =
...
module.exports = { dropElementToDocument, dropElementToExcel };
